...
public class LoginTramitadorViewController {
	...
	public void login() throws IOException, ClassNotFoundException {
		...
		Tramitador Comprador = new Tramitador();

		Comprador.setUsuario(usuario);
		Comprador.setContraenia(password);

		if (validarTexto(usuario) && validarTexto(password)) {
			if (modelFactoryController.validarIngresoTramitador(usuario, password) != null) {
				Tramitador anuncianteActual = modelFactoryController.obtenerTramitadorActual(Comprador);
				modelFactoryController.tramitadorActual = anuncianteActual;
				mostrarMensaje("Has ingresado correctamente ", null, " Bienvenido Tramitador",
						Alert.AlertType.INFORMATION);
				Stage stage = (Stage) this.btnIngresar.getScene().getWindow();
				stage.close();
				aplicacion.abrirTramitadorTramites();
			} else {
