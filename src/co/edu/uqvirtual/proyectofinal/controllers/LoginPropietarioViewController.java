...
public class LoginPropietarioViewController {
	...
	void hyper_RemenberPass_Action_Login(ActionEvent event) throws IOException {
		aplicacion.recuperarContrasenaPropietario();

	}

	@FXML
	void ingresarUsuario(ActionEvent event) throws IOException, ClassNotFoundException {
		login();
	}
	...
	public void login() throws IOException, ClassNotFoundException {
		...
		propietario.setContraenia(password);

		if (validarTexto(usuario) && validarTexto(password)) {
			if (modelFactoryController.validarIngresoPropietario(usuario, password) != null) {
				Propietario anuncianteActual = modelFactoryController.obtenerPropietarioActual(propietario);
				modelFactoryController.propietarioActual = anuncianteActual;
